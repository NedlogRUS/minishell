...
/*   Updated: 2023/08/26 18:31:41 by vtavitia         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	set_pipe(int pipes[1000][2], int i, int lines, int hd, int in)
{
	(void) in;
	if (i == 0)
	{
...
// 		}
// 		set_pipe(pipes, i, lines, 1, (*mh)->in);
// 		// // dup2((*mh)->in, STDIN_FILENO);
// 		// dup2((*mh)->screen, STDOUT_FILENO);
// 		if (ft_tokenlstsize(tmp->token) && (check_redir_exist((*mh)->token)))
// 		{
// 			printf("IN RED\n");
// 			print_tokens(tmp->token);
